...
int wolfram_f (const gsl_vector * x, void *params,
              gsl_vector * f)
{
  double a = ((struct wparams *) params)->a;
  double b = ((struct wparams *) params)->b;
  double c = ((struct wparams *) params)->c;
  double d = ((struct wparams *) params)->d;

  const double x0 = gsl_vector_get (x, 0);
  const double x1 = gsl_vector_get (x, 1);

  //  x^2+y^2-1=0 -> x^2+y^2-a=0
  // (x-2)^2+(y-1)^2-4=0 -> (x-b)^2+(y-c)^2-d=0

  const double y0 = x0*x0 + x1*x1 - a;
  const double y1 = (x0-b)*(x0-b) + (x1-c)*(x1-c) - d; 
...
/*
 *  Wolfram Example
 *  Solver can find both roots depending on the starting point!
 */
int main (void)
{
  ...
  const size_t n = 2;
  struct wparams p = {1.0, 2.0, 1.0, 4.0};
  gsl_multiroot_function f = {&wolfram_f, n, &p};

  double x_init[2] = {3.0, 3.0};
